    ...
    return (
        <div className="max-w-7xl mx-auto mt-8">
            <ul className="flex m-4 ml-0">
                {menuArray.map((e,i)=>{
                    return (
                        <li key={i} onClick={()=>{setIsActive(i); setCateGory(menuType[i])}} className={`${isActive === i ? 'bg-orange-500 text-white' : 'bg-white text-black'} cursor-pointer mr-4 border dark:bg-[#272929] dark:text-[#ebf4f1]  py-2 px-5 rounded-md`}>{e}</li>
                    )
                })}                
            </ul>
            <div className="mt-8">
                            ...
                            <div className="bg-white mb-8 pt-12 group px-8 pb-16 rounded-md flex border flex-wrap" key={i}>
                                ...
                                <div className="basis-[52%] group-even:order-2 xl:group-even:order-1 pt-10">
                                    ...
                                    <ul className="flex justify-center mt-6">
                                        <li className="py-1 px-8 border rounded-md text-sm mr-4"><a href={e.origianl} target="_blank">Orginal</a></li>
